...
export default function GunMasterySlider({ challengeMaxValue, challengeId, weapon }) {
    ...
    // on an async function needed for Mastery Tracker live update
    // necessary for not too many api requests and updating the tracker live on completion not on refresh
    useEffect(() => {
        if (send) {
            const asyncFunctionNecessaryForTrackerUpdate = async () => {
                const response = await storeProfileData()
                // console.log(response);
                response && checkProfileData();
            }
            asyncFunctionNecessaryForTrackerUpdate()
        }
    }, [send]);
